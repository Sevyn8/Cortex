...
### SD1 — Sub-phase structure: 7 sub-phases (D.1 → D.6, with D.4.5 inserted post-D.4 gate evidence)

- **Decision:** D.1 prototype gate / D.2 key rotation workflow / D.3 HTTP API / D.4 TF module + key-rotation-queue / **D.4.5 provisioning worker** / D.5 IAM authz / D.6 convention §7 + Slice D close. (D.4.5 inserted 2026-05-08 mid-stream — see "D.4.5 lineage" note below.)
...
- **D.4.5 lineage:** D.4 gate evidence (`docs/planning/d4-gate-evidence.md`) made operationally visible that the provisioning worker referenced at `tenants.ts:670` — always a known TODO ("actual worker function lands in sub-phase 4.3" predates this plan) — was blocking 3 D.3 deferred-skip happy paths + the §7.4 contract evidence. Inserting D.4.5 between D.4 and D.5 captures the work formally without renumbering D.5/D.6 or extending the close commit's scope. Mid-stream insert pattern matches D.0.5 precedent (post-spike ADR codification, `a685294`).
- **Trade-off:** 7 sub-phases is two more than F02 planning's "4-5" estimate. The +2 covers: (a) D.1's pass/fail gate adds non-trivial measurement + cleanup work that conflated with D.2 would muddy the gate signal; (b) D.4's TF + queue work at slice-end is heavier than Slice C's TF sub-phase (which only added env wiring; Slice D adds a new TF module); (c) D.4.5's provisioning worker emerged from D.4 gate evidence and is a real implementation step (4–6 hr). Acceptable.
- **Reference:** Slice C precedent (7 sub-phases including 7.1 audit). F01 Slice C planning ran 10 sub-phases (smaller-step granularity). 7 is in range.
...
- **Decision:** No fixed deadline. Effort estimate: **~4.5–6 working days at solo-dev pace for D.1 → D.6**, matching the sub-phase plan's 30–45 hr nominal total (revised: 24–36 hr at original plan; 28–42 hr at D.4.5 insert; 30–45 hr at D.4.5 close, when D.6's row absorbed D.4.5's deferred GCS-factory-seam + cascadeOverrides DI + 3 happy-path tests + §7.4 contract evidence work). 2–3 days remained achievable as a compressed best-case for the original D.1 → D.6 path. Either way, longer than Slice C's 1 day because of TF module scope + measurement work + 12-endpoint test surface + the deferred-into-D.6 fold-in.
...
| D.4.5 | Provisioning worker — `tenants.provisionWorker` Cloud-Tasks-only worker route | 4–6 hr   | Build the provisioning worker route and the workflow function it dispatches to. The worker advances a tenant through the provisioning state machine (Slice A defined the state machine; this brings up the worker that drives transitions). Pairs with the existing `provisioning-queue` Cloud Tasks queue (Slice A TF). Cloud Tasks dispatches POSTs to the worker route; OIDC validation per ADR-LIFECYCLE-001 §3 (same pattern as D.2's key-rotation worker route). Convention §7.4 written/extended to cover provisioning state-machine transitions. Discovered via D.4 gate evidence (`docs/planning/d4-gate-evidence.md`) — `tenants.ts:670` was always a known TODO; the gate evidence made the absence operationally visible (3 deferred D.3 happy paths + §7.4 contract evidence both depend on this worker existing).                                                                                                                                                                                                                                          | (a) D.3 deferred-skip happy paths re-enabled: 3 skipped tests (offboard / terminate / force-terminate happy paths) flip from skip to active and pass; (b) §7.4 contract evidence: post-provisioning, `POST /v1/tenants/:id/terminate` returns 202 if grace elapsed, 409 with §7.4 contract message otherwise — testable end-to-end; (c) Cloud Tasks dispatch from `tenants.provision` actually invokes the worker; no orphaned tasks in the queue. | D.1, D.2, D.3, D.4                                                                                                                                         |
...
| D.6   | Convention §7 finalize + Slice D close commit + D.4.5 deferral fold-in        | 4–6 hr   | Convention §7 review + drift fixes + final structural pass (analogous to Slice C 7.7). CLAUDE.md `apps/<workload>-api/` workspace-layout convention addition (per SD5). status.md update: P1.2 F02 Slice D ✓; F02 closes; P1.3 F03 unblocked. Slice D close commit `feat(F02-Slice-D): closes Slice D — key rotation + HTTP API + TF + IAM authz`. **D.4.5 deferrals fold-in (added 2026-05-08 at D.4.5 close):** (i) `__setStorageClientFactoryForTesting` (KMS-style) in `@cortex/blob-storage` so HTTP wrapper happy-path tests can run without live GCS; (ii) route-level `cascadeOverrides?` build option on `BuildV1TenantRoutesOptions` threading through offboard/terminate/force-terminate handlers; (iii) re-enable the 3 D.3 deferred-skip happy paths (1 skip-flip in `offboard.spec.ts` + 2 new in `terminate.spec.ts` / `force-terminate.spec.ts`); (iv) §7.4 contract evidence — assert `POST /v1/tenants/:id/terminate` 202 if grace elapsed, 409 with §7.4 contract message otherwise, exercised end-to-end at the HTTP wrapper layer with stubbed GCS. | (a) §7 has 6+ subsections matching §6's 7-subsection density (or close); (b) status.md reflects Slice D closure + F02 closure + P1.3 readiness; (c) CI green on close commit; (d) **D.4.5 fold-in**: GCS factory seam shipped in `@cortex/blob-storage`; route-level `cascadeOverrides?` DI seam shipped; 3 happy-path tests active + passing; §7.4 terminate-before-grace contract evidence captured.                                             | D.1, D.2, D.3, D.4, D.4.5, D.5                                                                                                                             |

**Total nominal:** 30–45 hours = ~4.5–6 working days at solo-dev pace. Cushion at the high end for D.3's 12-endpoint test surface + D.4's TF module work + D.4.5's worker + state-machine-transition wiring + D.6's expanded scope (D.4.5 deferral fold-in). (Was 28–42 hr at D.4.5 plan-insert; D.6's estimate grew 2–3 hr → 4–6 hr at D.4.5 close to absorb the GCS-factory-seam + cascadeOverrides DI + 3 happy-path tests + §7.4 contract evidence work that D.4.5 deferred per HOLD-#2 reconciliation.)
