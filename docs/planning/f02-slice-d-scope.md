...
- **Decision:** No fixed deadline. Effort estimate: **~4–6 working days at solo-dev pace for D.1 → D.6**, matching the sub-phase plan's 28–42 hr nominal total (revised upward from 24–36 hr after D.4.5 was inserted; see Sub-phase plan note). 2–3 days remained achievable as a compressed best-case for the original D.1 → D.6 path; the inserted D.4.5 (provisioning worker discovered via D.4 gate evidence) adds 4–6 hr. Either way, longer than Slice C's 1 day because of TF module scope + measurement work + 12-endpoint test surface.
...
**Total nominal:** 28–42 hours = ~4–6 working days at solo-dev pace. Cushion at the high end for D.3's 12-endpoint test surface + D.4's TF module work + D.4.5's worker + state-machine-transition wiring. (Was 24–36 hr before D.4.5 was inserted; the +4–6 hr captures the provisioning worker per `f02-slice-d-scope.md` D.4.5 row.)
