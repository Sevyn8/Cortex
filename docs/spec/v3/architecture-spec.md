...
Foundation and project topology: the cross-product topology is an open decision in ADR-INFRA-008 (Proposed), weighing one common platform project against keeping the three isolated projects and sharing only the event spine (via a thin shared-services project, or via DIS). Isolation between the identity, data, and platform planes is treated as a BFSI-review asset, so the isolation-preserving options are favored; the choice is joint with Sanjeev at GR. Regardless of option, repos stay separate (no repo merge) and cross-stack consumption is by name. The current per-product topology (sevyn8-cortex-\*, ithina-retail-dis, ithina-retail-admin) and the discovery are recorded in docs/architecture/infra-shared-resource-register.md; naming and deploy rules in docs/architecture/infra-naming-contract.md and docs/runbooks/deploy-order.md.
