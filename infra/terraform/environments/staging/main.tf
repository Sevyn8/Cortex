...
# Staging-specific: min_instances=1 (eliminate cold-start from staging hot
# path so e2e tests aren't first-deploy-flaky).
...
module "tenant_lifecycle_shared" {
  ...
  # deferred per roadmap §2.5a (BILLING_DISABLED on staging until
  # billing is re-attached).
