...
Last updated: 2026-05-08 (F02 Slice C closed 2026-04-28 — `135c9da`. F02 Slice D in flight: D.0 spike `cd285d6`; D.0.5 ADR-HTTP-001 `a685294`; Slice D Planning `3d29027`; D.1 Hono prototype `5236544` with Conditions 2 + 3 verified; D.2 `tenants.rotateKeys` + worker route `6fcc2b5`; D.3 HTTP API `c451703` with 294 active + 1 skipped tests passing; D.4 TF + IAM gap closure + firewall internal-egress `547b7ec` (dev applied; staging+prod deferred billing); D.5 invoker IAM + 3-case integration test + convention §7.7 (dev applied). Plan-insert: D.4.5 provisioning worker (post-D.4 gate evidence; not yet built). Next: D.6 (Slice D close) + D.4.5.)
...
- [ ] P1.2 F02 Tenant Lifecycle — IN FLIGHT (Slices A/B/C closed; Slice D in-flight)
  ...
  - [ ] Slice D — Key rotation + HTTP API + Cloud Run TF — IN FLIGHT (D.1 ✓ + D.2 ✓ + D.3 ✓ + D.4 ✓ + D.5 ✓; next: D.6 + D.4.5 (plan-inserted post-D.4 gate evidence; not yet built))
