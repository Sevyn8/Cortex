...
## 2026-06-11: v3 re-baseline package landed

The Cortex v3 re-baseline package landed in `docs/spec/v3/` (brd, architecture-spec, specification, reconciliation, plan), re-baselining spec v2.2 on the live estate: DIS as the data plane, Customer Master as the identity plane. Two ADRs were extracted from reconciliation.md, both Status: Proposed pending joint ratification with Sanjeev at gate GR: ADR-SCOPE-010 (DIS is the data plane) and ADR-IDENTITY-001 (Customer Master implements AC01, AC02, AC03). Annotated tag `v2-final` marks the v2.2-era build before the re-baseline. cortex_v2.2.docx is frozen as the historical record per specification.md section 17.

Next milestone: Phase R. Resolve the `[verify]` flags in the Module Disposition Register (reconciliation.md section 2), ratify both ADRs with Sanjeev at gate GR, run the salvage audit, and create the contracts repo. v2.2-era module builds are paused pending gate GR. `docs/future-roadmap.md` and `docs/deviations.md` predate v3 and are queued for Phase R reconciliation against `docs/spec/v3/plan.md`; until then, where they conflict with the v3 package, the v3 package governs.
...
- [x] P1.6 Feature Flags — module CLOSED 2026-05-10. 3 slices shipped: A core surface (`@cortex/feature-flags` package + 3-discriminated-union flag schemas + `isEnabled` / `getVariant` server APIs + per-key tier-walk merge over F04 substrate + own per-process LRU TTL=30s + 4 build-prompt flags registered eagerly + SHA-256 percentage rollout); B HTTP endpoint + client shim (`apps/feature-flags-api` Hono service serving `GET /v1/feature-flags?userId=...` bulk fetch + `createFeatureFlagsClient` non-React shim with 30s polling + diff-notify subscription); C admin UI stub (`packages/feature-flags/admin-stub/index.html` vanilla HTML+JS) + convention doc (`docs/architecture/feature-flags.md`) + module wrap-up. Day-of lineage: PRs #13 (scoping; D1-D8 locks + 3 slice shells) → #14 (Slice A) → #15 (Slice B; §1.19 runWithBoundTenantClient extraction trigger surfaced) → #16 (Slice C + module close; this PR). 68/68 tests stable across 2 packages (61 in `@cortex/feature-flags` + 7 in `@cortex/feature-flags-api`); 121/121 config-plane regression-stable; 84/84 foundation regression-stable. Gate evidence: `docs/planning/feature-flags-gate-evidence.md`. **Per-evaluation audit DEVIATION** from build-prompt literal documented at convention §5 (per-flag-change auditability via F04 lifecycle satisfies the spirit; per-evaluation untenable at scale). **Downstream queue:** SCR-04 Configuration screen replaces Slice C HTML stub (Phase 2 + ADR-FE-001 React infra); AC01 (P2.1) wires real auth on `apps/feature-flags-api` (Phase 2; trust-the-header `// TODO(AC01)` in Phase 1); A02 champion/challenger consumer of `agents.planogram.v2-model` (per build-prompt §3316); workspace-wide flag-rollout rule (build-prompt §399 + CLAUDE.md line 740) — every future capability is a P1.6 consumer.
